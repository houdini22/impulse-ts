...
export class OptimizerAdam extends AbstractOptimizer {
  ...
  adam(layer: Layers, learningRate: number, t: number): void {
    ...
    const wCorrected = getComputation().execute("elementWiseDivideNumber", layer.vW, 1 - Math.pow(beta1, t)) as Matrix;
    ...
    const sCorrected = getComputation().execute(
      "sqrt",
      getComputation().execute("elementWiseMultiplyNumber", layer.cW, 1 - Math.pow(beta2, t)) as Matrix
    ) as Matrix;
    ...
    const wCorrected2 = getComputation().execute("elementWiseDivideNumber", layer.vb, 1 - Math.pow(beta1, t)) as Matrix;
    ...
    const sCorrected2 = getComputation().execute(
      "sqrt",
      getComputation().execute("elementWiseDivideNumber", layer.cb, 1 - Math.pow(beta2, t)) as Matrix
    ) as Matrix;
