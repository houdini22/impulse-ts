  ...
  for (let row = 0; row < m1.rows; row++) {
    for (let col = 0; col < m2.cols; col++) {
      ...
      for (let k = 0; k < m1.cols; k++) {
        result.data[index] += m1.data[row * m1.cols + k] * m2.data[k * m2.cols + col]
      }
    }
  }
...
export const forEach = (m1: Matrix, callback: (x: number) => number): Matrix => {
...
export const colwise = (m: Matrix, callback: (colVector: Matrix) => number): Matrix => {
